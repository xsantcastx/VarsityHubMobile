...
export const User = {
  ...
  ban: (id: string) => {
    ...
    return httpPost('/users/' + encodeURIComponent(id) + '/ban', {});
  },
  unban: (id: string) => {
    if (!id || id === 'undefined' || id === 'null') throw new Error('[User.unban] Invalid user ID');
    return httpPost('/users/' + encodeURIComponent(id) + '/unban', {});
  },
  getFull: (id: string) => {
  ...
  approveFollowRequest: (followerId: string) => {
    if (!followerId || followerId === 'undefined' || followerId === 'null')
      throw new Error('[User.approveFollowRequest] Invalid user ID');
    return httpPost(`/users/${encodeURIComponent(followerId)}/follow-request/approve`, {});
  },
  denyFollowRequest: (followerId: string) => {
    if (!followerId || followerId === 'undefined' || followerId === 'null')
      throw new Error('[User.denyFollowRequest] Invalid user ID');
    return httpPost(`/users/${encodeURIComponent(followerId)}/follow-request/deny`, {});
  },
  postsForProfile: (
    id: string,
    opts: {
...
export const Post = {
  ...
  trendingPage: async (cursor?: string | null, limit: number = 20) => {
    try {
      ...
      console.log(
        '[Post.trendingPage] Trending endpoint not available, falling back to recent posts'
      );
      const q: string[] = [];
      if (cursor) q.push('cursor=' + encodeURIComponent(cursor));
      if (limit) q.push('limit=' + String(limit));
...
export const Organization = {
  ...
  requestToJoin: (organizationId: string, message?: string, role?: string) =>
    httpPost(`/organizations/${encodeURIComponent(organizationId)}/join-requests`, {
      message,
      role,
    }),
  ...
  rejectJoinRequest: (requestId: string, reason?: string) =>
    httpPost(`/organizations/join-requests/${encodeURIComponent(requestId)}/reject`, { reason }),
};
...
export const Advertisement = {
  ...
  review: (id: string, action: 'approve' | 'reject', note?: string) =>
    httpPost('/ads/' + encodeURIComponent(id) + '/review', { action, note }),
