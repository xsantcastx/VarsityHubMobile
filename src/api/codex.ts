...
export async function invokeCodex(
  ...
  try {
    ...
    const timeoutPromise = new Promise<never>((_, reject) =>
      setTimeout(
        () => reject(new Error(`Codex request timeout after ${timeoutMs}ms - stream disconnected`)),
        timeoutMs
      )
    );
    ...
    if (
      ...
      console.log(`[codex] Retrying in ${RETRY_DELAY_MS}ms... (${retries} attempts left)`);
...
export async function invokeCodexStream(
  ...
  try {
    ...
    const timeoutId = setTimeout(() => {
      if (!completed) {
        throw new Error(`Stream timeout after ${timeoutMs}ms - stream disconnected`);
      }
    }, timeoutMs);
    ...
    onChunk(`[Error: Unable to process stream. ${error?.message}. Please try again.]`);
  }
}
