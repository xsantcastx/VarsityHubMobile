...
export async function uploadFileWithProgress(
  ...
  if (!finalMimeType || finalMimeType === 'application/octet-stream') {
    ...
    const ext = name
      .toLowerCase()
      .match(/\.(jpg|jpeg|png|gif|webp|heic|heif|mp4|mov|avi|mkv)$/)?.[1];
    if (ext) {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          ...
          reject(
            new Error(`Server returned non-JSON response: ${xhr.responseText.substring(0, 100)}`)
          );
        }
      } else {
        ...
        try {
          err.data = JSON.parse(xhr.responseText);
        } catch (parseError) {
...
export async function uploadFile(
  ...
  while (attempt <= retries) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Server returned non-JSON response (HTTP ${res.status}): ${text.substring(0, 100)}...`
        );
      }
      if (!res.ok) {
      ...
      console.error(
        ...
        totalAttemptsForLog,
        '|',
        isAbort ? 'timeout' : err?.message || err
      );
  ...
  if (lastErr instanceof TypeError && lastErr.message === 'Network request failed') {
    throw new Error(
      'Network error: unable to reach upload endpoint. Check your internet connection and server status.'
    );
  }
  // Check for server-side connection pool exhaustion
  if (lastErr?.status === 500 && lastErr?.data?.message?.includes('maxRetries')) {
    throw new Error(
      'Server is temporarily overloaded. Please try again in a moment. (Connection pool limit reached)'
    );
  }
