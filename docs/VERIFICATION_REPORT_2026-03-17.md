...
| Recipient configurable                   | ✅     | Uses `ADMIN_EMAILS` first entry, fallback `support@varsityhub.app` |
...
**Production check:** Ensure `ADMIN_EMAILS` is set on Railway (e.g. `support@varsityhub.app`) and email delivery works (SendGrid configured).
