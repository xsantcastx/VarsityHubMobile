...
## Release configuration follow-up: Android Stripe initialization

The read-only release check of the published runtime 1.0.5 configuration found an empty bundled Stripe publishable key. Android's root StripeProvider therefore skipped SDK initialization. The existing ad-calendar fallback fetched a valid public key from `/payments/config`, but only checked the string; it never delivered that key to the SDK. Web ad checkout uses a server-created Checkout URL, and iOS ads use Apple IAP, so those paths do not depend on the Android provider. A read-only production config request returned HTTP 200 with a live public key present, payments enabled/configured and webhook configuration present; no credential value was printed or changed.

The correction exports the public `initStripe` API through the existing Stripe adapters and calls it only when the calendar resolved a missing/invalid bundled key through the existing server fallback. The call occurs before `/payments/create-payment-sheet`. An unavailable/noncallable SDK method or rejected initialization reaches the existing checkout error handling before any intent is created. The normal bundled-key provider path stays intact. iOS/web adapters keep native Stripe initialization unavailable, and their checkout branches never call it. No second provider, config cache, native API or server/schema change was added.

The installed Stripe SDK's `initStripe` queues native configuration rather than returning the native initialization promise. Awaiting this public API establishes JavaScript call ordering, not proof of completed native initialization. The following PaymentSheet initialization still handles native errors; a real Android-device checkout remains a separate release verification item.

Before/after proof executes the actual calendar handler extracted from source, with provider/UI boundaries mocked. Before the fix, two of eight assertions failed: no SDK call occurred for the server fallback, and an intent was created despite the injected SDK failure. After the fix, all eight pass. They cover delayed API dispatch ordering, initialization rejection, missing/server-failed config, normal bundled-key checkout, iOS Apple, complimentary web, and paid web. Six adapter checks cover forwarding the key, missing/noncallable SDK methods, propagated rejection and iOS/web boundaries. Seven existing IAP UI checks also pass: **three targeted suites, 21 passing checks**. These are supplemental results after the 319-suite aggregate, not a second full-suite run.

Both complete client and server TypeScript checks passed after this correction. Changed files pass Prettier. ESLint reports zero errors; existing adapter `any` warnings and the repository's test-file ignore notice remain. Evidence under `/tmp/varsityhub-remediation-2026-09-05/`: `stripe-runtime-before.json/.log`, `stripe-runtime-after.json/.log`, `stripe-adapter-final.json/.log`, `stripe-runtime-client-tsc.log`, `stripe-runtime-server-tsc.log`, and `stripe-runtime-eslint.log`.

The release owner separately corrects the production export environment. `app.config.js` writes the public key and DSN directly from export-process environment variables into Expo extra; `config/env.ts` reads those extras through dynamic keys. Expo only inlines statically referenced `process.env.EXPO_PUBLIC_*` properties, so a dynamically indexed config lookup does not repair an empty manifest key. Sentry additionally has a direct static DSN lookup and cannot be diagnosed solely from an empty manifest extra. `EXPO_PUBLIC_NODE_ENV` sets the reported Sentry environment; it does not change Metro's `__DEV__` flag or the Apple/Google billing rails. [Expo environment-variable rules](https://docs.expo.dev/guides/environment-variables/), [EAS Update environment selection](https://docs.expo.dev/eas/environment-variables/usage/).
