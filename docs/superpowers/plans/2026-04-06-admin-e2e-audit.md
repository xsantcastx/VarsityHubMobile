# Admin End-to-End Production Audit

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a single production audit script that tests every admin feature end-to-end against the live API, verifying that every flow that starts also ends, and detecting orphaned/dead states.

**Architecture:** One self-contained TypeScript script using native `fetch()` against the production API. Authenticates as admin via login, runs all tests sequentially, cleans up test data, outputs pass/fail report with issue details. Uses the same patterns as existing `auth-security-test.ts`.

**Tech Stack:** TypeScript, tsx runner, native fetch, dotenv
...
- [ ] **Step 2: Verify the scaffold compiles and runs**

Run: `npx tsx server/scripts/admin-e2e-audit.ts` (without env vars)
Expected: "Set ADMIN_EMAIL and ADMIN_PASSWORD env vars" and exit 1

- [ ] **Step 3: Commit scaffold**

```bash
git add server/scripts/admin-e2e-audit.ts
git commit -m "chore: scaffold admin e2e production audit script"
```
...
```bash
git add server/scripts/admin-e2e-audit.ts
git commit -m "feat(audit): phase 1 — admin auth & access control tests"
```
...
```bash
git add server/scripts/admin-e2e-audit.ts
git commit -m "feat(audit): phase 2 — dashboard & metrics validation"
```
...
async function testCoachApprovalFlow(token: string): Promise<void> {
  ...
  for (const coach of pendingCoaches.slice(0, 5)) {
    // Verify coach has preferences with role=coach
...
```bash
git add server/scripts/admin-e2e-audit.ts
git commit -m "feat(audit): phase 3 — coach approval flow verification"
```
...
```bash
git add server/scripts/admin-e2e-audit.ts
git commit -m "feat(audit): phase 4 — user management (warn/suspend/ban/unban)"
```
...
```bash
git add server/scripts/admin-e2e-audit.ts
git commit -m "feat(audit): phase 5 — reports & moderation"
```
...
```bash
git add server/scripts/admin-e2e-audit.ts
git commit -m "feat(audit): phase 6 & 7 — transactions & activity log"
```
...
async function testOrphanDetection(token: string): Promise<void> {
  ...
  // These are coaches who finished onboarding but never got approved
...
```bash
git add server/scripts/admin-e2e-audit.ts
git commit -m "feat(audit): phase 8 — orphan & dead state detection"
```
...
Search for `// PHASE_PLACEHOLDER` and delete those lines — they've been replaced by the real implementations.

- [ ] **Step 2: Run TypeScript check**

Run: `npx tsc --noEmit server/scripts/admin-e2e-audit.ts --skipLibCheck --esModuleInterop --module nodenext --moduleResolution nodenext --target es2022`
...
```bash
git add server/scripts/admin-e2e-audit.ts
git commit -m "feat: complete admin e2e production audit — 8 phases, orphan detection"
```
