...
**Note:** This verifies *code usage* — actual Railway values must be confirmed separately.
...
| Variable | Used In | Validation |
|----------|---------|------------|
| `APPLE_IAP_SHARED_SECRET` | `server/src/routes/payments.ts` (Apple receipt validation) | Server warns if missing; iOS IAP verification disabled without it |
...
| Item | Code Reference |
|------|----------------|
| `APPLE_IAP_SHARED_SECRET` | `server/src/routes/payments.ts` line 2230 |

**Status:** ✅ Server reads from env. Copy from App Store Connect → App Information → App-Specific Shared Secret.
...
| `jwt` | JWT_SECRET |
| `cloudinary` | CLOUDINARY_* (all 3) |
| `stripe` | STRIPE_SECRET_KEY |
| `sendgrid` | SENDGRID_API_KEY + required templates (VERIFICATION, PASSWORD_RESET) |
| `googleOAuth` | GOOGLE_OAUTH_CLIENT_IDS |
| `googleMaps` | GOOGLE_MAPS_API_KEY |

**Note:** `APPLE_IAP_SHARED_SECRET` is not in the health response; it's only checked at IAP verify time.
...
| IAP | ✅ | Confirm APPLE_IAP_SHARED_SECRET in Railway |
