...
### 7.1 Attach apex to Railway and www to Vercel

Do:

- Add the apex `ALIAS` record using the Railway target for `varsityhub.app`
- Add the `www` `CNAME` record using `cname.vercel-dns.com`

Pass:

- Railway shows the apex custom domain as verified
- Vercel shows `www.varsityhub.app` as verified
- HTTPS cert provisions successfully on both platforms
