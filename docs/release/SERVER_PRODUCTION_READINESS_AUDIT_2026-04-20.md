...
- **Gap:** Role-enforcement not uniform. Fans _can_ create teams if they select "coach" role later (no early enforcement)
...
- DOB grace window uses dob*set_at but doesn't check if \_this is the first DOB ever set*. Edge case: admin changes DOB, then user tries to change within 24h — rejects even though user never set it themselves **[LOW]**
