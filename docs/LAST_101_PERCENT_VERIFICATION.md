...
| 3   | Ad email: 3-tier fallback for pending review (template → plain text → direct SendGrid) | ✅ In `server` ad submit flow — email to ADMIN_EMAILS / support@varsityhub.app |
...
| 18    | Email to emancero@ for ad approval                                                                        | ✅ **Fixed (ac02e53).** Ad pending-review email with 3-tier fallback; ADMIN_EMAILS / fallback to support@varsityhub.app.                                                       |
