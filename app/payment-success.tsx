...
function PaymentSuccessScreen() {
    ...
    const verify = async () => {
      ...
      try {
        ...
        if (!paymentType) {
          if (!mounted) return;
          setError(
            'This payment link is invalid or incomplete. Please return to the app and try again.'
          );
          setLoading(false);
          return;
        }
        ...
        try {
          ...
          if (verificationAttempt >= currentMax - 1) {
            if (!mounted) return;
            setError(
              'We could not reach the server to confirm your payment. Please check your connection and try again.'
            );
            setLoading(false);
            return;
          }
        ...
        if (isAdPayment) {
          ...
          try {
            ...
            if (isAuthError(e)) {
              ...
              setShowSignInAction(true);
              setError(
                'Your purchase may have completed, but we need you to sign in with the purchasing account to confirm the subscription.'
              );
              setLoading(false);
              return;
            }
            ...
            if (verificationAttempt >= currentMax - 1) {
              if (!mounted) return;
              setError(
                'We could not reach the server to confirm your subscription. Please check your connection and try again.'
              );
              setLoading(false);
              return;
            }
  ...
  const isAdCurrentlyLive =
    isAdPayment && adDetails?.status === 'active' && adScheduleBucket === 'live';
  ...
  const adDeliveryBadgeStyle = isAdCurrentlyLive
    ? colorScheme === 'dark'
  ...
  const SuccessInfoBox = ({ lines }: { lines: string[] }) => (
    <View
      style={[styles.infoBox, { backgroundColor: colorScheme === 'dark' ? '#052e16' : '#F0FDF4' }]}
    >
      {lines.map(line => (
        <Text
          key={line}
            ...
            <View style={styles.centerInner}>
              ...
              {manualRetryCount < MAX_MANUAL_RETRIES ? (
                ...
                >
                  <Text style={styles.primaryBtnText}>
                    Try Again{' '}
                    {manualRetryCount > 0 ? `(${MAX_MANUAL_RETRIES - manualRetryCount} left)` : ''}
                  </Text>
                </Pressable>
              ) : (
                ...
                <Pressable
                  style={[styles.primaryBtn, { backgroundColor: theme.tint }]}
                  // Going BACK from payment-success lands on the paywall the
                  // user just paid on — reads as a failed payment and invites
                  // a duplicate purchase attempt. Clear the stack instead.
                  onPress={() => router.replace('/(tabs)/feed')} // nav-safe: purchase flow completion
                >
              ...
              {showSignInAction ? (
                <Pressable
                  style={[
                    styles.secondaryBtn,
                    { borderColor: theme.border, width: '100%', marginTop: 12 },
                  ]}
              ...
              <Pressable
                ...
                onPress={() => {
                  void Linking.openURL(
                    'mailto:customerservice@varsityhub.app?subject=Payment%20Verification%20Issue'
                  );
                }}
              >
              ...
              <Pressable
                style={styles.linkBtn}
                onPress={() => {
                  router.replace('/(tabs)/feed'); // nav-safe: purchase flow completion — same as above
                }}
              >
                <Text style={[styles.linkBtnText, { color: theme.mutedText }]}>
                  Continue to App
                </Text>
              </Pressable>
            </View>
          ...
          <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
            <View style={[styles.contentInner, isLargeScreen && styles.contentInnerLarge]}>
              ...
              <Animated.View style={[styles.checkWrap, { opacity: checkOpacity }]}>
                ...
                {isAdPayment ? (
                  <Text style={[styles.successSubtitle, { color: theme.mutedText }]}>
                    {adSuccessSubtitle}
                  </Text>
                ) : null}
              </Animated.View>
                    ...
                    <View style={styles.statusRow}>
                      ...
                      >
                        <Text
                          style={[
                            ...
                            { color: paidStatusBadgeStyle.textColor },
                          ]}
                        >
                          PAID
                        </Text>
                      ...
                      >
                        <Text
                          style={[
                            styles.statusBadgeText,
                            { color: adDeliveryBadgeStyle.textColor },
                          ]}
                        >
                        ...
                        <View style={styles.datesWrap}>
                          {adDetails.dates.sort().map(d => (
                            <View key={d} style={[styles.datePill, { borderColor: theme.border }]}>
                              ...
                              <Text style={[styles.datePillText, { color: theme.text }]}>
                                {formatDate(d)}
                              </Text>
                            </View>
                          ))}
