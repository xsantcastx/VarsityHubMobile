...
function Step3League() {
  ...
  const pageConfig = useMemo(() => {
    ...
    return {
      ...
      description:
        'Search for an organization to join existing teams, or create a new organization page.',
      alreadyExists: false,
    };
  ...
  const submitJoinRequest = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Request Sent!',
        `Your request to join "${selectedOrg.name}" has been sent for admin review. Press Continue to wait for approval.`,
        [{ text: 'OK' }]
      );
    } catch (error: any) {
      Alert.alert(
    ...
    try {
        ...
        const approvalStatus =
          typeof me?.approval_status === 'string'
            ...
            : typeof me?.preferences?.approval_status === 'string'
              ...
              : null;
        if (isPendingJoin || approvalStatus !== 'APPROVED') {
          router.replace({
            pathname: '/onboarding/pending-approval',
            params: {
              leagueName: resolvedOrgName || 'your organization',
      ...
      if (!docUrl && supportingDocumentUri) {
        ...
        try {
          ...
          Alert.alert(
            'Upload Failed',
            uploadErr?.message || 'Could not upload supporting document. Please try again.'
          );
          setSaving(false);
      ...
      if (
        ...
        Alert.alert(
          'Invalid Name',
          'Please use a clear, descriptive name without spam, links, or unsupported special characters.'
        );
      } else if (
