...
function CoachAgreementScreen() {
        ...
        const approvalStatus =
          typeof me?.approval_status === 'string'
            ...
            : typeof me?.preferences?.approval_status === 'string'
              ...
              : null;
        if (me?.preferences?.role !== 'coach' || approvalStatus !== 'APPROVED') {
          Alert.alert('Access Restricted', 'Only approved coach accounts can access this screen.');
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#0B1120' : '#F8FAFC' }]}>
      ...
      <View style={[styles.footer, { borderTopColor: C.border, opacity: checkingAccess ? 0.6 : 1 }]}>
        <Pressable
          style={[styles.acceptButton, { backgroundColor: C.tint }]}
          onPress={handleAccept}
          disabled={accepting || checkingAccess}
        >
          {accepting ? (
