...
function LeaguePendingApproval() {
  ...
  const checkApproval = useCallback(async () => {
    if (!orgId) {
      setCompletionError(
        'Organization setup is incomplete. Go back to league setup and select or create your organization.'
      );
      return;
    }
    try {
      ...
      if (isRejected) {
        ...
        try {
          ...
          const rejectionNotif = Array.isArray(page?.items)
            ? page.items.find(
                (n: any) =>
                  (n.type === 'COACH_REJECTED' || n.type === 'ORG_REJECTED') && n.meta?.reason
              )
            : null;
      ...
      if (me?.approval_status === 'APPROVED') {
        ...
        try {
          ...
          if (__DEV__)
            console.warn('[league-pending-approval] Failed to complete onboarding:', err);
        ...
        if (!completed) {
          setCompletionError(
            'VarsityHub has approved your organization, but account setup failed. Tap retry below.'
          );
          return;
        }
  ...
  const handleLogout = async () => {
    try {
      ...
      if (__DEV__)
        console.warn('[league-pending-approval] Logout failed:', (err as Error)?.message ?? err);
    }
      ...
      <View style={styles.content}>
        ...
        {rejected && rejectionReason ? (
          ...
          >
            ...
            >
              You can try creating a new league or continue as a fan. Contact support@varsityhub.app
              for questions.
            </Text>
          </View>
        ) : null}
        ...
        {!orgId ? (
          <>
            <Text
              style={[styles.supportText, { color: '#EF4444', marginTop: 0, marginBottom: 12 }]}
            >
        ...
        {timedOut && !approved && !rejected && orgId && (
          <>
            ...
            >
              This is taking longer than usual. We'll email you when your league is approved. You
