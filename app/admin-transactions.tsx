...
function AdminTransactionsScreen() {
  ...
  const loadData = useCallback(
    async (reset = false, showRefreshing = false) => {
      if (!isAdmin) return;
      ...
      <FlatList
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={() => void loadData(true, true)}
          ...
          <View style={{ padding: 16 }}>
            ...
            {summary && (
              <View style={styles.summaryRow}>
                <SummaryCard
                  title="Revenue"
                  value={formatAmount(summary.totalRevenue || summary.total_revenue || 0)}
                  color="#10B981"
                  icon="attach-money"
                />
                <SummaryCard
                  title="Completed"
                  value={summary.completedCount || summary.completed_count || 0}
                  color="#3B82F6"
                  icon="check-circle"
                />
                <SummaryCard
                  title="Pending"
                  value={summary.pendingCount || summary.pending_count || 0}
                  color="#F59E0B"
                  icon="schedule"
                />
                <SummaryCard
                  title="Failed"
                  value={summary.failedCount || summary.failed_count || 0}
                  color="#EF4444"
                  icon="error"
                />
              </View>
            ...
            <View style={styles.filterRow}>
              {filters.map(f => (
                ...
                >
                  <Text
                    style={{
                      color: filter === f.value ? 'white' : palette.text,
                      fontSize: 13,
                      fontWeight: '600',
                    }}
                  >
        ...
        renderItem={({ item }) => (
          <View style={[styles.txCard, { backgroundColor: cardBg, borderColor: cardBorder }]}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 6 }}>
              ...
              >
                <Text style={{ color: statusColor(item.status), fontSize: 10, fontWeight: '800' }}>
                  {item.status}
                </Text>
              </View>
              <Text style={[styles.txMeta, { color: palette.mutedText }]}>
        ...
        ListFooterComponent={() => (
          <>
            ...
            {hasMore && !loading && transactions.length > 0 && (
              <Pressable
                style={[styles.loadMore, { borderColor: cardBorder }]}
