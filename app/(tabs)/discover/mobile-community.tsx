...
export default function CommunityDiscoverScreen() {
  ...
  const load = useCallback(
    ...
    [authUser, loadGames, loadPersonalization]
  );
    ...
    <View>
      ...
      {unifiedSearchLoading ? (
        ...
        >
              ...
              {unifiedSearchResults.teams.map(t => (
                <Pressable
                  ...
                  onPress={() => {
                    setQuery('');
                    setUnifiedSearchResults(null);
                    void router.push(`/team-profile?id=${t.id}`);
                  }}
                >
                  <View style={styles.searchResultLeft}>
          ...
          unifiedSearchResults.teams.length === 0 &&
          unifiedSearchResults.organizations.length === 0 ? (
            <Text style={[styles.searchEmpty, { color: Colors[colorScheme].mutedText }]}>
              No results found
            </Text>
          ) : null}
        </View>
            ...
            {nearbyPeople.map(u => (
              ...
              >
                <View style={styles.personAvatar}>
                  {u.avatar_url ? (
                    <Image
                      source={{ uri: String(u.avatar_url) }}
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={{ paddingHorizontal: 16, paddingBottom: 8 }}>
            <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
              ...
              >
                <Ionicons name={'list'} size={24} color={Colors[colorScheme].tint} />
              </Pressable>
            </View>
          </View>
          ...
          {(() => {
            ...
            const allGamesWithCoords = games.filter(
                  ...
                  return (
                    <View style={styles.teamRow}>
                      <View style={styles.teamPill}>
                        <Text style={styles.teamPillText}>{labels.teamA}</Text>
                      </View>
                      <Text style={[styles.vsText, { color: Colors[colorScheme].mutedText }]}>
                        vs
                      </Text>
                      <View style={styles.teamPillAlt}>
                        <Text style={styles.teamPillAltText}>{labels.teamB}</Text>
                      </View>
                    </View>
                  );
                })()}
