...
export default function EventDetailScreen() {
  ...
  const rawId = params.id ?? params.eventId;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        try {
          const [userResult, status]: any = await Promise.all([
            Promise.resolve(user).catch(() => null),
  ...
  const toggleRsvp = async () => {
    ...
    try {
      ...
      if (e?.response?.status === 401 || e?.message?.includes('Unauthorized')) {
        ...
        Alert.alert('RSVP', "We couldn't update your RSVP. Try again in a moment.");
      }
    } finally {
      setRsvpBusy(false);
    }
        ...
        {event && !loading && (
          <View style={{ gap: 8 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}>
              <Pressable
                style={[styles.primaryBtn, rsvpBusy && { opacity: 0.6 }]}
