...
export default function PostDetailScreen() {
  const params = useLocalSearchParams<{ id?: string | string[]; postIds?: string | string[]; index?: string | string[] }>();
  ...
  const rawPostIds = typeof params.postIds === 'string' ? params.postIds : Array.isArray(params.postIds) ? params.postIds[0] : undefined;
  const rawIndex = typeof params.index === 'string' ? params.index : Array.isArray(params.index) ? params.index[0] : undefined;
  const postIdsArray = useMemo(() => {
    return rawPostIds ? rawPostIds.split(',').filter(Boolean) : rawId ? [rawId] : [];
  }, [rawPostIds, rawId]);
  const initialIndex = rawIndex ? parseInt(rawIndex, 10) : 0;
  
  // State for current post in swipe sequence
  const [currentPostIndex, setCurrentPostIndex] = useState(initialIndex);
  const currentPostId = useMemo(() => {
    return postIdsArray[currentPostIndex] || rawId;
  }, [postIdsArray, currentPostIndex, rawId]);
  ...
  const load = useCallback(async (postId?: string, showLoading = true) => {
    ...
    if (!targetId) {
      setError("We couldn't find this post. Go back and try opening it again.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!p || !p.id) {
        setError("We couldn't find this post. It may have been removed—go back and try another.");
        setLoading(false);
        return;
      }
      ...
      setError("We couldn't load this post. It may have been removed—go back and try another.");
  ...
  }, [load, rawId, rawPostIds, currentPostIndex]);
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Delete Comment', "We couldn't delete this comment. Try again.");
            }
          }
        }
      ]
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
                ...
                [
                  {
                    ...
                    onPress: async () => {
                      try {
                        ...
                        Alert.alert('Restore Expired', "The undo window has passed. Your post was deleted.");
                        router.back();
                      }
                    }
                  },
              ...
              Alert.alert('Delete Post', "We couldn't remove this post. Try again.");
            }
          }
        }
      ]
  ...
  const handleEditComment = async () => {
    ...
    try {
      ...
      Alert.alert('Update Comment', "We couldn't update this comment. Try again.");
    } finally {
