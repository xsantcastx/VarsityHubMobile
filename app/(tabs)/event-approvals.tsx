...
import { useEventApprovals, type PendingEvent } from '@/hooks/useEventApprovals';
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: Colors[colorScheme].background }]} edges={['bottom']}>
        ...
        <Pressable
          ...
          onPress={() => { // Role-based navigation
            if (me?.role === 'coach') { void router.push('/manage-season');
              ...
              void router.push('/create-fan-event');
            }
          }}
        >
          <Text style={[
      ...
      {sectionTab === 'approvals' ? (
        loading ? (
          ...
          <View style={[styles.loadingContainer, { padding: 32 }]}>
            ...
            <Text style={[styles.emptyTitle, { color: Colors[colorScheme].text, marginTop: 16, marginBottom: 24, textAlign: 'center' }]}>{error}</Text>
            <Pressable
      ...
      <QuickAddGameModal
        ...
        currentTeamName={(me as any)?.team?.name}
        currentTeamId={(me as any)?.team?.id}
        userRole={(me as any)?.role === 'coach' || (me as any)?.role === 'admin' ? 'coach' : 'fan'}
      />
    </SafeAreaView>
  );
}
