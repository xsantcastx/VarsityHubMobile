  ...
  useEffect(() => {
    if (!permissionGranted && !hasAutoSuggested && !gameId) {
      requestPermission().then((granted) => {
        if (!granted) {
          showLocationDeniedAlert('posts');
          setLocationError('Location helps suggest nearby events. You can enable it in Settings.');
        }
      }).catch(() => {
  ...
  const confirmPost = async () => {
    ...
    try {
      ...
      if (picked?.uri) {
        ...
        const res = await uploadMedia(picked.uri, name, mime);
