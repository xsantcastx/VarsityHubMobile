...
export default function CreateTeamScreen() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const userRole = (user as any)?.preferences?.role; // Already guaranteed coach above
      const userPlan = (user as any)?.preferences?.plan || 'rookie'; // Default to rookie if not set
      ...
      const teamCount = latestLimits?.owned_teams ?? (user as any)?._count?.teams ?? 0;
              ...
              { text: 'Cancel', style: 'cancel', onPress: () => setSubmitting(false) },
              {
                ...
                onPress: async () => {
                  try {
                    ...
                    // @ts-ignore
                    const { Subscriptions } = await import('@/api/entities');
  ...
  const proceedWithTeamCreation = async (_user?: any) => {
    try {
      ...
      if (logoUri) {
        try {
          const uploaded = await uploadMedia(logoUri, 'team-logo.jpg', 'image/jpeg');
