  ...
  const load = useCallback(async () => {
    ...
    try {
      const page: any = await PostApi.filterPage(
        { game_id: String(game_id), type: 'highlight' },
        null,
        24
      );
      ...
      {error ? (
        ...
        <FlatList
          ...
          renderItem={({ item: _item }) => (
            <Pressable
              onPress={() =>
                void router.push(`/post-detail?id=${encodeURIComponent(String(_item.id))}`)
              }
          ...
          ListEmptyComponent={
            !loading ? (
              <Text style={[styles.muted, { color: theme.mutedText }]}>No highlights yet.</Text>
            ) : null
          }
        />
      )}
