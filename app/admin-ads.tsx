...
type AdStatus = 'draft' | 'pending' | 'approved' | 'active' | 'rejected';
...
export default function AdminAdsScreen() {
  ...
  const bulkReject = async () => {
    ...
    Alert.alert('Reject Ads', `Reject ${selectedAds.size} ad${selectedAds.size > 1 ? 's' : ''}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            Alert.alert(
              'Success',
              `Rejected ${selectedAds.size} ad${selectedAds.size > 1 ? 's' : ''}`
            );
            setSelectedAds(new Set());
            setBulkMode(false);
            await load();
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                'Success',
                `Deleted ${selectedAds.size} ad${selectedAds.size > 1 ? 's' : ''}`
              );
              setSelectedAds(new Set());
              setBulkMode(false);
              await load();
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              ...
              <Pressable
                style={[
                  styles.btn,
                  styles.btnSecondary,
                  item.status === 'pending' ? { flex: 0, paddingHorizontal: 16 } : { flex: 1 },
                ]}
  ...
  return (
    ...
    >
              ...
              {bulkMode && (
                <>
                  ...
                  >
                    <Text
                      style={{
                        color: selectedAds.size > 0 ? '#22c55e' : '#9ca3af',
                        fontWeight: '600',
                      }}
                    >
                  ...
                  >
                    <Text
                      style={{
                        color: selectedAds.size > 0 ? '#f59e0b' : '#9ca3af',
                        fontWeight: '600',
                      }}
                    >
                  ...
                  >
                    <Text
                      style={{
                        color: selectedAds.size > 0 ? '#dc2626' : '#9ca3af',
                        fontWeight: '600',
                      }}
                    >
      ...
      <View style={{ flexDirection: 'row', padding: 12, gap: 8, backgroundColor: theme.surface }}>
        {(['all', 'pending', 'approved', 'active', 'rejected', 'draft'] as const).map(status => (
          <Pressable
            key={status}
      ...
      >
        ...
        >
          <View
            style={[styles.card, { backgroundColor: theme.card, width: '100%', maxWidth: 400 }]}
          >
            ...
            <TextInput
              ...
              placeholder={
                reviewModal?.action === 'approve'
                  ? 'e.g., Looks great! Payment link sent.'
                  : 'e.g., Banner image is too blurry, please re-upload.'
              }
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <Pressable
                style={[
                  ...
                  {
                    flex: 1,
                    backgroundColor: reviewModal?.action === 'approve' ? '#22c55e' : '#dc2626',
                  },
                ]}
