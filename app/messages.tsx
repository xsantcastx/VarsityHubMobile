...
export default function MessagesScreen() {
  ...
  const renderItem = ({ item }: { item: RawMessage }) => {
    ...
    const other = me?.email && (item.sender_email === me.email ? item.recipient_email : item.sender_email);
    const goThread = () => {
      if (item.conversation_id) router.push(`/message-thread?conversation_id=${encodeURIComponent(item.conversation_id)}`);
      else if (other) router.push(`/message-thread?with=${encodeURIComponent(String(other))}`);
    };
    return (
