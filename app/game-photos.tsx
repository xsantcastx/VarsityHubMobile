  ...
  const load = useCallback(async () => {
    ...
    try {
      const page: any = await PostApi.filterPage(
        { game_id: String(game_id), type: 'photo' },
        null,
        24
      );
      ...
      {error ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable
              onPress={() =>
                void router.push(`/post-detail?id=${encodeURIComponent(String(item.id))}`)
              }
          ...
          ListEmptyComponent={
            !loading ? (
              <Text
                style={{ color: Colors[colorScheme].mutedText, textAlign: 'center', marginTop: 16 }}
              >
                No photos yet
              </Text>
            ) : null
