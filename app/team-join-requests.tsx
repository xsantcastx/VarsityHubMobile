...
export default function TeamJoinRequestsScreen() {
  ...
  const handleReject = useCallback((request: JoinRequest) => {
    Alert.alert(
      'Decline Request',
      `Decline ${request.user.display_name}'s request to join?`,
      [
  ...
  const renderItem = useCallback(({ item }: { item: JoinRequest }) => {
    const isActing = acting === item.id;
    const initials = item.user.display_name?.slice(0, 2).toUpperCase() || '??';

    return (
      <View style={[styles.card, { backgroundColor: theme.surface || theme.background, borderColor: theme.border }]}>
        <View style={styles.cardHeader}>
          ...
          <View style={styles.userInfo}>
            <Text style={[styles.displayName, { color: theme.text }]}>{item.user.display_name}</Text>
            {item.user.username ? (
              <Text style={[styles.username, { color: theme.mutedText }]}>@{item.user.username}</Text>
            ) : null}
          </View>
        </View>
        ...
        <View style={styles.actions}>
          <Pressable
            style={[styles.approveButton, { backgroundColor: '#16A34A' }, isActing && styles.disabledButton]}
          ...
          <Pressable
            style={[styles.declineButton, { borderColor: theme.border }, isActing && styles.disabledButton]}
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]} edges={['bottom']}>
      <Stack.Screen
        options={{
          ...
          headerLeft: () => (
            <Pressable onPress={() => safeGoBack(router)} accessibilityRole="button" accessibilityLabel="Go back" style={{ paddingHorizontal: 8 }}>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          ...
          <Pressable style={[styles.retryButton, { backgroundColor: theme.tint }]} onPress={() => void loadRequests()}>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Ionicons name="people-outline" size={48} color={theme.mutedText} style={{ marginBottom: 12 }} />
...
const styles = StyleSheet.create({
  ...
  declineButton: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    paddingVertical: 10, borderRadius: 8, borderWidth: 1, gap: 6,
  },
