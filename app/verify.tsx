...
export default function VerifyScreen() {
  ...
  const params = useLocalSearchParams<{ devCode?: string | string[]; returnTo?: string }>();
  const devCodeParam = typeof params.devCode === 'string' ? params.devCode : Array.isArray(params.devCode) ? params.devCode[0] : undefined;
  ...
  const onVerify = async () => {
    ...
    try {
      ...
      try {
        ...
        try {
          const refreshed: any = await checkAuth();
          ...
          if (!isFan) {
            destination = returnTo || '/onboarding/step-1-role';
          } else {
