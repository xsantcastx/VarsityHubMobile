...
export default function ProfileScreen() {
  ...
  const { user: userFromHook, refresh: refreshUserFromHook } = useUser();
  ...
  const loadProfile = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      const currentUser: any = await checkAuth();
      ...
      if (!options?.silent) {
        if (e && e.status === 401) {
          ...
          setError("We couldn't load this profile. Pull to refresh.");
        }
  ...
  }, [activeTab, refreshPosts, refreshReplies, refreshUpvotes, me, viewingUserId]);
  ...
  const renderHeader = () => (
    <>
      ...
      <View style={styles.profileDetailsContainer}>
        {/* Edit Profile Button - Right-aligned (PDF: "move edit profile button to the right") */}
        {!viewingUserId || viewingUserId === currentUserId ? (
          <View style={[styles.usernameRow, { justifyContent: 'flex-end' }]}>
