...
function DMRestrictionsScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const me = (await getAuthSnapshot(checkAuth, user)) as any;
  ...
  const Row = ({ k, title, desc }: { k: Policy; title: string; desc: string }) => (
    <Pressable
      ...
      style={[
        styles.row,
        {
          ...
          borderColor: policy === k ? Colors[colorScheme].tint : Colors[colorScheme].border,
    ...
    >
      ...
      <Text
        style={[
          styles.badge,
          { color: policy === k ? Colors[colorScheme].tint : Colors[colorScheme].text },
        ]}
      >
        {policy === k ? (saving ? '...' : 'Selected') : 'Select'}
      </Text>
    </Pressable>
  );
