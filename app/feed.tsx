...
import { canShowGamePoll } from '@/utils/eventPresentation';
...
export default function FeedScreen() {
  ...
  const preloadVoteSummaries = useCallback(async (gameList: GameItem[]) => {
    const candidates = gameList
      .filter(game =>
        canShowGamePoll({ gameId: game?.id ? String(game.id) : null, eventType: game?.event_type })
      )
  ...
  const renderGameCard = useCallback(
    (gameItem: GameItem, isLive: boolean, testIDPrefix: string) => {
      ...
      const canShowVotePreview = canShowGamePoll({
        gameId: gameItem?.id ? String(gameItem.id) : null,
        eventType: (gameItem as any)?.event_type,
      });
      ...
      switch (item._t) {
        ...
        case 'followed_teams_post': {
          ...
          const media = resolvePostMedia({
            ...
            preview_url: post.preview_url || post.previewUrl || null,
          });
            ...
            <View style={{ paddingHorizontal: 16, marginBottom: 20 }}>
              ...
              >
                ...
                {media.displayImageUrl && (
                  <Image
                    source={{ uri: optimizeImageUrl(media.displayImageUrl, 400) }}
                    style={StyleSheet.absoluteFillObject}
                    contentFit="cover"
                  />
                )}
