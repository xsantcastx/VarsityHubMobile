...
import { Advertisement, Event, Game, Highlights, Notification as NotificationApi } from '@/api/entities';
...
const RSVPBadge = ({ gameItem, onRSVPChange }: { gameItem: any, onRSVPChange?: () => void }) => {
  ...
  const handleRSVP = async () => {
    ...
    try {
      ...
      Alert.alert('RSVP', "We couldn't update your RSVP. Try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const load = useCallback(async ({ silent = false }: { silent?: boolean } = {}) => {
    ...
    try {
      ...
      const [highlightsData, forFeedAds] = await Promise.all([
        ...
        Advertisement.forFeed(todayISO, undefined, 5).catch((err) => {
          if (__DEV__) console.warn('[Feed] Ads load failed:', err?.message || err);
          return null;
        }),
      ]);
      ...
      if (forFeedAds && Array.isArray((forFeedAds as any).ads)) {
        const raw = (forFeedAds as any).ads as any[];
        const list = raw.filter((a) => !!a && typeof a === 'object');
  ...
  const openInstagram = useCallback(async () => {
    ...
    try {
      ...
      if (canOpen) {
        ...
        Alert.alert('Instagram', "We couldn't open Instagram. Check your connection and try again.");
      }
    } catch (error) {
