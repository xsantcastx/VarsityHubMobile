---
name: apple-design
description: The design-principles and craft reference behind Apple-style interfaces — the *why*, plus the web/CSS *how*. Use for design decisions and reviews: the eight design foundations (purpose, agency, simplicity-not-minimalism, craft), typography theory (optical sizing, tracking, leading), spatial consistency, materials/depth reasoning, feedback and restraint, and the canonical motion constants (spring damping/response, momentum projection, rubber-banding). Also the how-to for web/marketing surfaces (CSS, backdrop-filter, Pointer Events). For IMPLEMENTING or reviewing motion/gesture code on a native React Native / Expo screen, use react-native-design instead (it maps these principles onto Reanimated 4 + Gesture Handler v2); pull this skill in alongside it only when a design principle or the canonical values are in question.
---
