/* eslint-disable */
// Pure Node.js seed to avoid TSX IPC in sandbox
...
async function main() {
  ...
  for (let i = 0; i < sportsContent.length; i++) {
    ...
    const post = await prisma.post.create({
      data: {
        title: c.title,
        content: c.content,
        type: c.media ? 'highlight' : 'update',
        media_url: c.media || null,
        upvotes_count: c.upvotes,
        author_id: authorId,
  ...
  const topPosts = postsCreated.slice(0, 5);
  ...
  if (gameIds.length > 0) {
    await prisma.story.create({
      data: { game_id: gameIds[0], user_id: u1.id, media_url: 'https://picsum.photos/seed/story1/640/360', caption: 'Warmups' },
    });
  }
  ...
  await prisma.teamMembership.create({ data: { team_id: t2.id, user_id: u2.id, role: 'coach' } });
  ...
  await prisma.organizationMembership.upsert({
    where: { organization_id_user_id: { organization_id: westhill.id, user_id: u1.id } },
    update: { role: 'owner' },
  ...
  await prisma.organizationMembership.upsert({
    where: { organization_id_user_id: { organization_id: greenwich.id, user_id: u2.id } },
    update: { role: 'owner' },
  ...
  await prisma.organizationMembership.upsert({
    where: { organization_id_user_id: { organization_id: stamfordSoccer.id, user_id: u3.id } },
    update: { role: 'owner' },
  ...
  await prisma.organizationMembership.upsert({
    where: { organization_id_user_id: { organization_id: darien.id, user_id: u1.id } },
    update: { role: 'admin' },
  ...
  await prisma.organizationMembership.upsert({
    where: { organization_id_user_id: { organization_id: newCanaanBaseball.id, user_id: u2.id } },
    update: { role: 'owner' },
  ...
  await prisma.organizationMembership.upsert({
    where: { organization_id_user_id: { organization_id: fairfieldPrep.id, user_id: u3.id } },
    update: { role: 'admin' },
  ...
  await prisma.organizationMembership.upsert({
    where: { organization_id_user_id: { organization_id: norwalkHockey.id, user_id: u1.id } },
    update: { role: 'member' },
  ...
  await prisma.promoCode.upsert({ where: { code: 'FREE100' }, update: {}, create: { code: 'FREE100', type: 'COMPLIMENTARY', enabled: true, max_redemptions: 100 } });
