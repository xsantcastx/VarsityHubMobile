...
import { sendAccountRecoveryEmail } from '../dist/lib/email.js';
...
(async () => {
  try {
    const ok = await sendAccountRecoveryEmail(to, name, date);
    console.log(JSON.stringify({ ok }));
  } catch (err) {
    console.error('Error sending account recovery:', err);
    process.exitCode = 1;
  }
})();
