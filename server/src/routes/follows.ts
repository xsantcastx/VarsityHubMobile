...
followsRouter.get('/teams', requireAuth as any, async (req: AuthedRequest, res) => {
  try {
    ...
    const follows = await prisma.teamFollow.findMany({ where: { user_id: userId }, include: { team: true }, orderBy: { created_at: 'desc' }, take: 500 });
    ...
    console.error('[follows] GET /teams error:', err);
