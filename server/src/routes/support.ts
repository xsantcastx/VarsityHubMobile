...
supportRouter.post('/contact', reportLimiter, asyncHandler(async (req: AuthedRequest, res) => {
...
supportRouter.post('/feedback', reportLimiter, asyncHandler(async (req: AuthedRequest, res) => {
  if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
