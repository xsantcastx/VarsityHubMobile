...
async function sendAdPaymentEmail({
  ...
  const perks = [`Ad #${adId}`, dates.length ? `Dates: ${dates.join(', ')}` : null].filter(
    Boolean
  ) as string[];
  try {
...
async function createMembershipCheckoutSession(
  ...
  const billableQuantity =
    chosen === 'veteran' && typeof teamCount === 'number' ? Math.max(0, teamCount - 2) : 1;
    ...
    : [
        {
          ...
          price_data: {
            currency: 'usd',
            unit_amount: chosen === 'veteran' ? 100 : 2999, // Veteran: $1.00/month per additional team, Legend: $29.99/year
            ...
            product_data: {
              ...
              description:
                chosen === 'veteran'
                  ...
                  : 'Legend plan - $29.99/year unlimited (fallback price)',
            },
          },
        },
      ];
  ...
  const appBase =
    process.env.APP_BASE_URL || process.env.EXPO_PUBLIC_API_URL || 'http://localhost:4000';
  ...
  const amount = chosen === 'veteran' ? 100 * billableQuantity : 2999; // Veteran: $1.00/month per additional team, Legend: $29.99/year
...
paymentsRouter.post(
  ...
  async (req: AuthedRequest, res) => {
    ...
    if (ad.status === 'archived') {
      return res.status(409).json({ error: 'Archived ads cannot be checked out' });
    }

    // Checkout is the canonical "submit for review" transition for ads.
    // Draft ads enter moderation before they can ever go live.
    if (ad.status === 'draft') {
      await prisma.ad.update({
        where: { id: String(ad_id) },
        data: { status: getAdStatusForCheckoutSubmission(ad.status) },
      });
    }
    ...
    try {
      ...
      if (typeof team_count !== 'number' || team_count < 3) {
        return res.status(400).json({
          error: 'Invalid total team count. Minimum 3 total teams required for Veteran plan.',
        });
      }
      ...
      if (billable === 0) {
        return res
          .status(400)
          .json({ error: 'No billable teams (only 2). Remain on Rookie plan instead.' });
      }
  ...
  if (ad_id && Array.isArray(dates) && dates.length) {
    ...
    try {
      await prisma.$transaction(
        async tx => {
          ...
          const adRecord = await tx.ad.findUnique({
            where: { id: ad_id },
            select: { user_id: true, target_zip_code: true, payment_status: true, status: true },
          });
          if (!adRecord) {
          ...
          await tx.ad.update({
            ...
            data: {
              payment_status: 'paid',
              status: getAdStatusAfterSuccessfulPayment(adRecord.status),
            },
          });
          await tx.adReservation.createMany({
      ...
      if (e?.slotFull) {
        ...
        console.error(
          '[payments] SLOT_FULL: ad dates overbooked after payment — manual refund required',
          {
            ad_id,
            full_dates: e.dates,
            session_id: session.id,
