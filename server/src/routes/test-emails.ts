...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await logAdminActivityFromReq(
      ...
      'email',
      String(to),
      'Sent password reset test email',
      { to }
    );
    return res.json({ ok });
  })
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await logAdminActivityFromReq(
      req,
      'TEST_EMAIL_TEAM_INVITE',
      'email',
      String(to),
      'Sent team invite test email',
      { to, role, teamName, organizationName }
    );
    return res.json({ ok });
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await logAdminActivityFromReq(
      req,
      'TEST_EMAIL_ORG_INVITE',
      'email',
      String(to),
      'Sent organization invite test email',
      { to, role, organizationName }
    );
    return res.json({ ok });
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await logAdminActivityFromReq(
      ...
      'email',
      String(to),
      'Sent billing notice test email',
      { to, type, planName, amount }
    );
    return res.json({ ok });
  })
);
