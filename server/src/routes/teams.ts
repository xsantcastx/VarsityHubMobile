    ...
    try {
      ...
      if (currentUserId) {
        ...
        const hasPrivilegedAccess =
          !!isAdmin || !!orgMembership || isManagementRole(membership?.role);
        ...
        async tx => {
          ...
          if (userPlan === 'veteran') {
            ...
            try {
              ...
              const sc = new stripeLib.default(process.env.STRIPE_SECRET_KEY || '', {
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    if (userPlan === 'rookie' || !userPlan) {
      const ownedTeamsCount =
        teamCountSource === 'org' && orgIdForTeamCount
    ...
    if (!organizationId) {
      ...
      try {
        ...
        console.error(
          '[teams] Organization auto-creation failed for team. Aborting team creation.',
            ...
            if (subId) {
              try {
                ...
                if (ownedTeamsInTx >= paidQty) {
                  throw new Error(
                    `TEAM_LIMIT_EXCEEDED:Subscription covers ${paidQty} teams but you already own ${ownedTeamsInTx}`
                  );
                }
              } catch (err: any) {
                if (err?.message?.startsWith('TEAM_LIMIT_EXCEEDED:')) throw err;
                throw new Error(
                  'TEAM_LIMIT_EXCEEDED:Unable to verify subscription. Please try again.'
                );
              }
            }
          }
        ...
        if (managers.length > 0) {
          ...
          for (const m of managerUsers) {
            ...
            sendInvitationDeclinedEmail({
              ...
              console.warn('[teams] sendInvitationDeclinedEmail failed:', err?.message ?? err)
            );
          }
        }
      } catch (notifErr) {
