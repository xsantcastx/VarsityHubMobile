...
export async function reverseGeocode(lat: number, lng: number): Promise<ReverseGeocodeResult> {
  ...
  if (best) {
    ...
    const byType = (t: string) =>
      comps.find((c: any) => Array.isArray(c.types) && c.types.includes(t));
...
export async function geocodeZip(
  zip: string,
  country?: string | null
): Promise<{ lat: number | null; lng: number | null; country_code: string | null }> {
  const key = getApiKey();
...
export function getCountryFromReqOrPrefs(
