...
export function resolveFixture(
  ...
  const latitude = hasFixtureCoords ? fixture.venue_lat! : (useHomeFallback ? (home?.venue_lat ?? null) : null);
  const longitude = hasFixtureCoords ? fixture.venue_lng! : (useHomeFallback ? (home?.venue_lng ?? null) : null);

  const venueName = hasFixtureCoords
    ? (fixture.venue_name ?? null)
    : (useHomeFallback ? (fixture.venue_name ?? home?.venue_name ?? null) : (fixture.venue_name ?? null));
  const venueAddress = hasFixtureCoords
    ? (fixture.venue_address ?? null)
    : (useHomeFallback ? (fixture.venue_address ?? home?.venue_address ?? null) : (fixture.venue_address ?? null));
