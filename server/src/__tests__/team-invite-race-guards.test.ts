...
describe('team invite race guards', () => {
  it('accept route only transitions pending invites and returns 409 when already processed', () => {
    ...
    expect(teamsSrc).toMatch(
      /teamsRouter\.post\(\s*'\/invites\/:inviteId\/accept'[\s\S]*?teamInvite\.updateMany\(\{/
    );
  ...
  it('decline route also uses a pending-only updateMany guard', () => {
    expect(teamsSrc).toMatch(
      /teamsRouter\.post\(\s*'\/invites\/:inviteId\/decline'[\s\S]*?teamInvite\.updateMany\(\{/
    );
    expect(teamsSrc).toMatch(
      /teamsRouter\.post\(\s*'\/invites\/:inviteId\/decline'[\s\S]*?status:\s*'pending'/
    );
  });
