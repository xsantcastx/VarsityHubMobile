...
const approvalServiceSrc = readFileSync(join(process.cwd(), 'src', 'lib', 'approvalService.ts'), 'utf8');
const organizationRoutesSrc = readFileSync(join(process.cwd(), 'src', 'routes', 'organizations.ts'), 'utf8');
...
describe('organization final-state guards', () => {
  it('approveOrganization refuses already-rejected leagues and keeps the DB guard aligned', () => {
    ...
    expect(approveOrgSlice).toContain("if (org.status === 'rejected') return { error: 'Organization already rejected'");
    expect(approveOrgSlice).toContain("where: { id: orgId, admin_approved: false, status: { not: 'rejected' } }");
  });
  ...
  it('rejectOrganization refuses already-approved leagues and keeps the DB guard aligned', () => {
    ...
    expect(rejectOrgSlice).toContain("if (org.admin_approved) return { error: 'Organization already approved'");
    expect(rejectOrgSlice).toContain("where: { id: orgId, status: { not: 'rejected' }, admin_approved: false }");
  });
