...
describe('POST /me/data-export', () => {
  ...
  it('202 when authenticated — queues an export (or 409/503 in test env without queue)', async () => {
    const res = await request(app)
      .post('/me/data-export')
      .set('Authorization', `Bearer ${userToken}`);
    // 202 = queued, 409 = already in-flight, 503 = job queue unavailable in test
    expect([202, 409, 503]).toContain(res.status);
    if (res.status === 202) {
