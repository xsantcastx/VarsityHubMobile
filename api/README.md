...
| **user.ts** | `User.me()`, `updateMe`, `upgradeToCoach`. |
| **games.ts** | Games CRUD, votes, posts, media, stories, result, approve. |
| **teams.ts** | Teams CRUD, managed, limits, follow, members, invites, create, transfer. |
| **organizations.ts** | Orgs CRUD, mine, follow, members, create, invites, join-requests, coaches. |
| **posts.ts** | Posts list/create/update/delete, trending, reactions. |
| **events.ts** | Events CRUD, list, RSVPs. |
| **payments.ts** | Checkout, subscription, finalize, Apple/Google receipt verification. |
| **notifications.ts** | List, mark read. |
| **messages.ts** | List, send. |
| **organizations.ts** | See above. |
| **misc.ts** | Report, Support, Advertisement, Search, Highlights. |
...
- Use **entities** for domain types: `import { User, Game } from '@/api/entities'`.
- Use **domain modules** for calls: `User.me()`, `Game.list()`, `Organization.mine()`.
