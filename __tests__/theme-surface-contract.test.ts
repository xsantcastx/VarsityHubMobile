...
describe('theme surface contract', () => {
  it('keeps the flagged readability regressions on theme-driven surfaces', () => {
    ...
    const zipPreview = read('components', 'ZipCodeMapPreview.tsx');
    const seasonStats = read('app', 'season-stats.tsx');

    expect(actionModal).toContain('backgroundColor: opt.isDestructive ? destructiveButtonBackground : theme.surface');
    expect(actionModal).toContain('borderColor: opt.isDestructive ? destructiveButtonBorder : theme.border');
    ...
    expect(zipAlternatives).toContain('{ backgroundColor: theme.background, borderColor: theme.border }');
    expect(zipAlternatives).toContain('backgroundColor: colorScheme === \'dark\' ? theme.border : \'#D1D5DB\'');
    ...
    expect(zipPreview).toContain('[styles.mapWrapper, { backgroundColor: theme.surface }]');
    expect(zipPreview).not.toContain("backgroundColor: '#f5f5f5'");
