...
export function nextIncompleteStep(
  ...
  // Step 9: Features (optional) - fans skip this to avoid confusion (PDF: "Fan onboarding shouldn't see this page")
  if (!isCoach && !isStepComplete(9, state, role)) {
    return ONBOARDING_STEPS.STEP_10_CONFIRMATION.index; // Skip step 9 for fans
  }
  if (isCoach && !isStepComplete(9, state, role)) {
